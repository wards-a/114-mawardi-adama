<x-caption title="Tambah Flag Baru" class="my-5" />
<div id="flag" class="p-4 shadow-md sm:rounded-lg dark:bg-gray-800">
    <form action="{{ route('flag.store') }}" method="POST">
        @csrf
        <x-form.text-input name="name" label="Nama" placeholder="Recommended" class="mb-6" />
        <x-form.textarea name="description" label="Deskripsi" placeholder="Flag yang digunakan untuk mengelompokkan produk..." rows="3" class="mb-6" />
